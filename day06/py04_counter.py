...
root.title(f'카운트 예제') # 윈도우 창 제목변경
...
label.pack(side=TOP, pady=20) # 윈도우에 패킹(필수!)
# 버튼, command 파라미터 - 이벤트 함수를 정의
buttonUp = Button(root, text='카운트 증가', font=myfont,
                  command = countUP) # countUP이라는 함수가 마우스 클릭떄마다 실행
buttonUp.pack(side=LEFT, padx=20, pady=20)
buttonInit = Button(root, text='초기화', font=myfont, command = countInit) # countUP이라는 함수가 실행
